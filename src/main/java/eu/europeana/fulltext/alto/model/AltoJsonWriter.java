...
public class AltoJsonWriter extends AbsAltoVisitor
{
    ...
    public void visit(TextString word)
    {
        ...
        if ( lang != null ) { _ps.print(",\"lang\":\"" + lang + "\""); }

        printStype(word.getStyle());

        Float confidence = word.getConfidence();
        if ( confidence != null ) { _ps.print(",\"confidence\":" + confidence); }
    ...
    public void visit(TextSpace space) { _ps.print("{\"space\":null}"); }

    public void visit(TextHyphen hyphen) { _ps.print("null"); }
    ...
    private void printStype(TextStyle style)
    {
        ...
        if ( size != null ) { _ps.print("\"size\":" + size); }
