...
public class AltoLanguageProcessor implements AltoPageProcessor
{
    ...
    private static Logger LOG = Logger.getLogger(AltoLanguageProcessor.class);

    private Map<String,Integer> _langs       = new LinkedHashMap();
    private boolean             _acceptNull;

    public AltoLanguageProcessor() { this(false); }
    ...
    private void detect(TextBlock block, String def)
    {
        for ( TextNode<?> tn : block )
        {
            String lang = getLang(tn.getLanguage(), def);
                 if ( tn instanceof TextBlock ) { detect((TextBlock)tn, lang); }
            else if ( tn instanceof TextLine  ) { detect((TextLine)tn , lang); }
        }
    }
    ...
    private void detect(TextLine line, String def)
    {
        ...
        {
            if ( !(te instanceof TextString) ) { continue; }
    ...
    private Stat getPredominantLang(boolean acceptNull)
    {
        ...
        if ( acceptNull ) { return set.first(); }

        Stat chosen = set.pollFirst();
        if ( chosen.lang != null ) { return chosen; }
    ...
    private void update(TextNode<? extends TextElement> node, String def)
    {
        for ( TextElement e : node )
        {
            ...
            { 
                ...
                if ( def.equals(lang) ) { le.setLanguage(null); }
            }

            if ( e instanceof TextNode ) { update((TextNode)e, def); }
        }
    }
