...
{
    private static Logger LOG = Logger.getLogger(AltoParser.class);

    private Context           _context   = null;
    private AltoPageProcessor _processor = null;

    public AltoParser() { _processor = new AltoPageProcessorImpl(); }
                           ...
                           , Attributes attr) throws SAXException
    {
        ...
        if ( localName.equals("SP") ) { _context.newSpace(); return; }
        if ( localName.equals("HYP") )
        { 
            _context.newHyphen(attr.getValue("CONTENT")); return;
        }
                                             ...
                                             , TextString w)
    {
        ...
        if ( !subsType.equals("HypPart2") ) { return null; }

        SubstitutionHyphen subs = _context.getCurrentSubs();
        if ( subs == null ) { subs = _context.newSubstitution(subsText); }
        subs.setWord2(w);
        w.setSubs(subs);
    ...
    private void setStyleRefs(TextStyle style, String str)
    {
        ...
        {
            if ( _context.hasParagraphStyle(ref) ) { continue; }

            TextStyle s = _context.getStyle(ref);
            if ( s != null ) { style.copyStyle(s); }
        }
    }
    ...
    private void setStyleTypes(TextStyle style, String str)
    {
        ...
        { 
            ...
            if ( textType != null ) { style.addType(textType); }
        }
    }
    ...
    private Float getStyleSize(Attributes attr)
    {
        ...
        try { return ( value != null ? Float.parseFloat(value) : null); }
        catch (NumberFormatException e) { return null;                  }
    }
    ...
    {
        try { return ( value == null ? null : Float.parseFloat(value)); }
        catch (NumberFormatException e) { return null; }
    }
    ...
    {
        private Collection         _parStyles = new HashSet();
        private AltoPage           _page;
        private SubstitutionHyphen _subs;
        private MediaReference     _ref;
        ...
        protected TextStyle getCurrentStyle()
        {
            ...
            if ( node instanceof AltoPage ) { return new TextStyle(); }
        ...
        protected TextStyle getStyle(String id)
        {
            ...
            if ( style == null ) { LOG.warn("Unknown style: " + id); }

            return style;
        }

        protected AltoPage getPage() { return _page; }
        ...
        protected void newSpace() { peek().add(TextSpace.SINGLETON); }

        protected TextHyphen newHyphen(String text)
        { 
            TextHyphen hyphen = new TextHyphen(text);
            peek().add(hyphen);
            if ( _subs != null ) { _subs.setHyphen(hyphen); }
            return hyphen;
        }
